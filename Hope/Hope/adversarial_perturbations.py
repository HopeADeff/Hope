...
def save_optimized(
    ...
    for attempt_quality in range(current_quality, 84, -2):
        ...
        if size_kb <= max_size_kb:
            if verbose:
                println(
                    f"STATUS: Saved JPEG (quality={attempt_quality}, {size_kb:.1f}KB)"
                )
            return int(size_kb)

    image.save(output_path, format="JPEG", quality=85, subsampling=2, optimize=True)
    size_kb = Path(output_path).stat().st_size / 1024
    if verbose:
...
class AdversarialProtector:
    def __init__(
        self, intensity: float = 0.30, iterations: int = 150, verbose: bool = True
    ):
    ...
    def apply_pgd_attack(self, img_tensor: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(self.iterations):
            ...
            if self.verbose and (i + 1) % 15 == 0:
                println(
                    f"STATUS: Iter {i+1}/{self.iterations} | Loss: {loss.item():.4f}"
                )
    ...
    def protect_image(
        ...
        try:
            ...
            if self.verbose:
                ...
                println(
                    f"STATUS: Intensity={self.intensity}, Iterations={self.iterations}"
                )

            perturbed = self.apply_pgd_attack(img_tensor)

            protected_array = (
                perturbed.squeeze(0).permute(1, 2, 0).cpu().detach().numpy()
            )
            ...
            if self.verbose:
                println(
                    f"STATUS: File size: {input_size_kb:.1f}KB → {output_size_kb}KB"
                )
...
def protect_image(
    ...
    protector = AdversarialProtector(
        intensity=intensity, iterations=iterations, verbose=verbose
    )
    return protector.protect_image(
        input_path,
        output_path,
