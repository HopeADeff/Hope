...
def cmd_protect(args):
    ...
    try:
        if args.target_style:
            println(
                f"STATUS: Using Glaze-style protection (target: {args.target_style})"
            )
            ...
            protector = GlazeStyleProtector(
                ...
                model_type=getattr(args, 'model_type', 'sd-v1-5')
            )
            ...
            println("STATUS: Using Nightshade protection")
...
def main():
    ...
    protect_parser = subparsers.add_parser(
        'protect', help='Protect image from AI'
    )
    ...
    protect_parser.add_argument(
        '--target-style',
        choices=['abstract', 'impressionist', 'cubist', 'sketch', 'watercolor'],
        help='Glaze-style protection'
    )
    protect_parser.add_argument(
        '--nightshade',
        action='store_true',
        help='Use Nightshade protection'
    )
    protect_parser.add_argument(
        '--source-concept',
        default='artwork',
        help='Source concept (for Nightshade)'
    )
    protect_parser.add_argument(
        '--target-concept',
        default='noise',
        help='Target concept (for Nightshade)'
    )
    protect_parser.add_argument(
        '--target',
        default='',
        help='Target description (for adversarial)'
    )
    protect_parser.add_argument(
        '--target-file', help='Read target from file'
    )
    protect_parser.add_argument(
        '--intensity',
        type=float,
        default=0.30,
        help='Protection intensity'
    )
    protect_parser.add_argument(
        '--iterations',
        type=int,
    ...
    protect_parser.add_argument(
        '--max-file-size', type=int, help='Max file size in KB'
    )
    protect_parser.add_argument(
        '--model-type',
        choices=['sd-v1-5', 'sdxl'],
        default='sd-v1-5',
        help='Model type: sd-v1-5 (default) or sdxl'
    )
    ...
    parser.add_argument(
        '--model-type', choices=['sd-v1-5', 'sdxl'], default='sd-v1-5'
    )
